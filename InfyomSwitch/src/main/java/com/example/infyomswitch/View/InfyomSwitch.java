package com.example.infyomswitch.View;
...
import com.example.infyomswitch.R;


public class InfyomSwitch extends ToggleableView {
    private int padding;
    ...
    private int labelOnColor;
    private String labelOff;
    private int labelOffColor;
    ...
    private Drawable thumbOnIcon;
    private Drawable thumbOffIcon;


    public InfyomSwitch(Context context) {
        super(context);
        initView();
    }


    public InfyomSwitch(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView();
    ...
    public InfyomSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initView() {
        ...
        this.colorOff = Color.parseColor("#FF424F"); //#FF424F
        this.colorDisabled = Color.parseColor("#D3D3D3");
        this.labelOnColor = Color.parseColor("#FFFFFF");
        this.labelOffColor = Color.parseColor("#000000");
    }
    ...
    private void initProperties(AttributeSet attrs) {
        ...
        for (int i = 0; i < N; ++i) {
            ...
            } else if (attr == R.styleable.Toggle_textOnColor) {
                labelOnColor = tarr.getColor(R.styleable.Toggle_textOnColor,Color.parseColor("#ffffff"));
            }else if (attr == R.styleable.Toggle_textOffColor) {
                labelOffColor = tarr.getColor(R.styleable.Toggle_textOffColor,Color.parseColor("#000000"));
            ...
            } else if (attr == R.styleable.Toggle_thumbOnIcon) {
                thumbOnIcon = tarr.getDrawable(R.styleable.Toggle_thumbOnIcon);
            } else if (attr == R.styleable.Toggle_thumbOffIcon) {
                thumbOffIcon = tarr.getDrawable(R.styleable.Toggle_thumbOffIcon);
            }
        }
    }
...
//      Drawing Switch Labels here
        ...
        if (isOn) {
            ...
            int lableOnColor = Color.argb(alpha, Color.red(labelOnColor), Color.green(labelOnColor), Color.blue(labelOnColor));
            lablePaint.setColor(lableOnColor);
            paint.setColor(onColor);


            float centerX = (width - padding - ((padding + (padding >>> 1)) + (thumbRadii << 1))) >>> 1;
            canvas.drawText(labelOff, (padding + (padding >>> 1)) + (thumbRadii << 1) + centerX - (paint.measureText(labelOff) / 2), (height >>> 1) + textCenter, lablePaint);

            alpha = (int) (((thumbBounds.centerX() - (width >>> 1)) / (thumbOnCenterX - (width >>> 1))) * 255);
            alpha = (alpha < 0 ? 0 : (alpha > 255 ? 255 : alpha));
            int offColor = Color.argb(alpha, Color.red(colorOff), Color.green(colorOff), Color.blue(colorOff));
            int lableOffColor = Color.argb(alpha, Color.red(labelOffColor), Color.green(labelOffColor), Color.blue(labelOffColor));
            lablePaint.setColor(lableOffColor);
            paint.setColor(offColor);

            int maxSize = width - (padding << 1) - (thumbRadii << 1);

            centerX = (((padding >>> 1) + maxSize) - padding) >>> 1;
            canvas.drawText(labelOn, padding + centerX - (paint.measureText(labelOn) / 2), (height >>> 1) + textCenter, lablePaint);
        } else {
        ...
        {
            // Draw the "off" icon with appropriate alpha
            if (thumbOffIcon != null && thumbOnIcon !=null) {
                int alpha = (int) (((thumbBounds.centerX() - thumbOffCenterX) / (thumbOnCenterX - thumbOffCenterX)) * 255);
                alpha = Math.max(0, Math.min(255, alpha)); // Clamp alpha between 0 and 255
                thumbOffIcon.setAlpha(alpha);

                int iconWidth = (int) (this.getMeasuredWidth() / 2.77);
                int iconHeight = (int) (this.getMeasuredWidth() / 2.77);
                int left = (int) thumbBounds.centerX() - iconWidth / 2;
                int top = (int) thumbBounds.centerY() - iconHeight / 2;
                int right = left + iconWidth;
                int bottom = top + iconHeight;
                thumbOffIcon.setBounds(left, top, right, bottom);

                thumbOffIcon.draw(canvas);
            }

            // Draw the "on" icon with appropriate alpha
            if (thumbOnIcon != null && thumbOffIcon !=null) {
                int alpha = (int) (((thumbOnCenterX - thumbBounds.centerX()) / (thumbOnCenterX - thumbOffCenterX)) * 255);
                alpha = Math.max(0, Math.min(255, alpha)); // Clamp alpha between 0 and 255
                thumbOnIcon.setAlpha(alpha);

                int iconWidth = (int) (this.getMeasuredWidth() / 2.77);
                int iconHeight = (int) (this.getMeasuredWidth() / 2.77);
                Log.d("£££", "onDraw: width  "+iconWidth+" | "+iconHeight+" "+this.getMeasuredWidth()+"| "+this.getMeasuredHeight());
                int left = (int) thumbBounds.centerX() - iconWidth / 2;
                int top = (int) thumbBounds.centerY() - iconHeight / 2;
                int right = left + iconWidth;
                int bottom = top + iconHeight;
                thumbOnIcon.setBounds(left, top, right, bottom);

                thumbOnIcon.draw(canvas);
            }
        }
